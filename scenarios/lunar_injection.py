...
try:
    ...
    warnings.warn(f"poliastro not available: {e}. Using simplified calculations.", stacklevel=2)
...
class LunarInjectionCalculator:
    ...
    def __init__(self, epoch: Time | None = None):
        """
    ...
    def calculate_injection_vector(
        ...
        if target_altitude_km < 200:
            warnings.warn(
                f"Target altitude {target_altitude_km} km is very low. "
        ...
        if target_relative_velocity_ms > 20000:
            warnings.warn(
                f"Target relative velocity {target_relative_velocity_ms/1000:.1f} km/s "
        ...
        try:
            ...
            warnings.warn(
                f"Lambert solve failed: {e}. Using patched-conic approximation.",
                stacklevel=2,
            )
...
def run_validation_tests():
    ...
    print("This is the ABSOLUTE orbital velocity, NOT the relative velocity.")
    print("Relative velocity is measured in the station/stream frame.")
