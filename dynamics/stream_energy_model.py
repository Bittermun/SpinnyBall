...
def compute_stream_energy_budget(
    ...
    mp: float,           # kg
    u: float,            # m/s
    theta_bias: float,   # rad - bias deflection angle
    ...
    Args:
        ...
        theta_bias: Bias deflection angle (rad)
    ...
    """
    # Total stream KE
    total_KE = 0.5 * N_packets * mp * u**2
    
    # Station deflection power drain
    # P = F * u * sin(theta) ≈ F * u * theta for small theta
    # The effective deflection angle for station-keeping force:
    # F = lambda * u^2 * sin(theta), where lambda = mp / spacing
    # For a single packet interaction, we use the bias angle directly
    # Power drain is the rate at which KE is removed from the stream
    # P_drain = F * u * sin(theta_bias)
    P_station = F_station * u * np.sin(theta_bias) * n_stations
    
    # Eddy heating drain (total across all packets)
    P_eddy_total = eddy_power_per_packet_W * N_packets
    
    # Slingshot replenishment
    # Each slingshot packet gains dv, adding KE = 0.5 * mp * ((u+dv)^2 - u^2) ≈ mp * u * dv
    if n_slingshot_packets > 0 and slingshot_dv_per_cycle > 0:
    ...
    if P_net < 0:
        ...
        packets_per_second = abs(P_net) / KE_per_packet
