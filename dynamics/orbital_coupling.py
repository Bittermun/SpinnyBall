...
try:
    ...
    from poliastro.constants import J2_earth, M_earth, R_earth
    from poliastro.perturbations import SRP, AtmosphericDrag, J2Perturbation
...
class OrbitalState:
    ...
    epoch: float | None = None  # Epoch time (seconds)
    ...
    def get_orbital_elements(self) -> OrbitalElements:
        ...
        try:
            ...
            if hasattr(self, '_state') and self._state is not None:
                ...
                raise ValueError(f"Failed to get orbital elements: {e}") from e
...
def compute_eclipse(r_eci: np.ndarray, sun_position: np.ndarray | None = None) -> bool:
