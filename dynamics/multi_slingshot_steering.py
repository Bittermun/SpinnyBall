...
try:
    from dynamics.flux_gyroscopic_dynamics import (  # noqa: F401
        FluxGyroscopicCoupledSystem,
        FluxGyroState,
    )
    from dynamics.gravity_slingshot import (
        GravityBody,  # noqa: F401
        GravitySlingshotOptimizer,
        SlingshotTrajectory,
    )
except ImportError:
    from gravity_slingshot import GravitySlingshotOptimizer, SlingshotTrajectory
...
class SteeringEvent:
    ...
    body_target: str | None = None  # Target body for alignment
    ...
    def simulate_mission(
        ...
        while state.mission_elapsed_time < self.max_time and slingshot_idx < len(sequence):
            ...
            if distance_to_target < soi_radius and state.current_phase != MissionPhase.SLINGSHOT:
                ...
                self._execute_slingshot(state, target)
