...
class FluxGyroscopicCoupledSystem:
    ...
    def __init__(self, config: FluxGyroConfig, flux_model: BeanLondonModel | None = None):
        """
    ...
    def simulate_coupled_response(
        ...
        duration: float,
        dt: float = 0.001,
        disturbance_schedule: Callable[[float], tuple[np.ndarray, np.ndarray]] | None = None,
    ...
    def _simulate_adaptive(
        ...
        if not sol.success:
            warnings.warn(f"Integration failed: {sol.message}", stacklevel=2)
