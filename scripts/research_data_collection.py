...
def run_converged_monte_carlo(
    ...
    def stream_factory():
        return create_stream_with_nodes(n_nodes=n_nodes)
...
def run_convergence_study(fault_rate: float, n_realizations_list: list[int]) -> dict:
    ...
    for n in n_realizations_list:
        ...
        runner = CascadeRunner(config)
        def stream_factory():
            return create_stream_with_nodes(n_nodes=10)
        results = runner.run_monte_carlo(stream_factory)
...
def main():
    ...
    logger.info("Files:")
    logger.info("  - reproducibility_manifest.json")
    logger.info("  - t3_fault_rate_sweep.json")
    logger.info("  - convergence_study.json")
    logger.info("\nKey Findings:")
