...
from sgms_v1 import P as DEFAULT_PARAMS
from sgms_v1 import simulate_multi_pass_accumulation
...
def test_multi_pass_accumulation_warning_large_n():
    ...
    test_params = get_test_params()
    simulate_multi_pass_accumulation(n_passes=10001, params=test_params, verbose=False)


def test_multi_pass_accumulation_verbose_false():
    """Test that verbose=False suppresses output."""
    test_params = get_test_params()
    simulate_multi_pass_accumulation(n_passes=5, params=test_params, verbose=False)
...
def test_multi_pass_accumulation_insufficient_variance():
    ...
    results['std_delta_vx'] * np.sqrt(results['n_passes'])
    np.std(results['cumulative_delta_vx'])
...
def test_multi_pass_accumulation_progress_logging():
    ...
    test_params = get_test_params()
    simulate_multi_pass_accumulation(n_passes=15, params=test_params, verbose=True)
